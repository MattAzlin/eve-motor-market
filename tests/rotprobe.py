...
MUTATIONEN = [
    ...
    ('Runplaner zeigt Treibstoff erst nach den Reaktionen',
     ...
     '        for stage in ("unrefined", "reaction_1", "reaction_2", "fuel", "component", "end"):   # MUTATION',
     'Runplaner zeigt Treibstoff VOR den Reaktionen'),
    # Unrefined muss VOR den Intermediates stehen (Nutzer 19.09.2026).
    ('Unrefined-Stufe steht hinter den Intermediates',
     'eve_trader/industry.py',
     '    for stage in ("fuel", "unrefined", "reaction_1", "reaction_2", "component", "end"):\n        jlist = stages[stage]',
     '    for stage in ("fuel", "reaction_1", "unrefined", "reaction_2", "component", "end"):   # MUTATION\n        jlist = stages[stage]',
     'aa153 Unrefined-Stufe liegt zwischen Fuel und Intermediate'),
    ('Unrefined-Jobs landen bei den Intermediates',
     'eve_trader/industry.py',
     '        if j["activity"] == REACTION and j.get("is_unrefined"):\n            stages["unrefined"].append(j)\n',
     '        if False:   # MUTATION\n            stages["unrefined"].append(j)\n',
     "b7w der Runplaner hat die Stufe '2. Unrefined reactions' und den Block direkt danach"),
    ...
    ('Gold-Menue schreibt wieder von Hand in den Wagen',
     ...
     '''                _qty_add = store.add_shopping(tid, nm, 1, 0, 0,
                                              source="daytrade") or 1   # MUTATION''',
...
''',
    ...
    # STANDARD AN (Nutzer-Entscheid 19.09.2026).
    ('Der Corp-Hangar ist standardmaessig AUS',
     'eve_trader/config.py',
     '    "use_corp": True,\n',
     '    "use_corp": False,\n',
     'aa362 Corp-Hangar ist standardmaessig AN, Implantat-Erkennung auch'),
    ...
    # Regel 3: eine angefangene Portion darf nichts liefern.
    ('Reprocessing zaehlt angefangene Portionen mit',
     'eve_trader/industry.py',
     '    portionen = menge // portion\n',
     '    portionen = -(-menge // portion)   # MUTATION\n',
     'aa364 volle Portionen, je Portion abgerundet'),
    # Regel 3: je Portion abrunden, nie runden.
    ('Reprocessing rundet je Portion statt abzurunden',
     'eve_trader/industry.py',
     '            je_portion = int(math.floor(int(q) * ausbeute))\n',
     '            je_portion = int(round(int(q) * ausbeute))   # MUTATION\n',
     'aa364 volle Portionen, je Portion abgerundet'),
    # Eine Ausbeute ueber 100 % ist ein Fehler, keine Zahl.
    ('Reprocessing nimmt eine Ausbeute ueber 100 % hin',
     'eve_trader/industry.py',
     '    if portion < 1 or menge < portion or not (0.0 < ausbeute <= 1.0):\n',
     '    if portion < 1 or menge < portion or not (0.0 < ausbeute):   # MUTATION\n',
     'aa364 Ausbeute ueber 100 % ist keine Zahl, sondern ein Fehler'),
    # Der Leser muss die Portion mitnehmen - sonst rechnet alles mit 1.
    ('reprocess_map ignoriert die Portionsgroesse',
     'eve_trader/industry.py',
     '                e = out.setdefault(t, {"portion": portion.get(t, 1), "out": {}})\n',
     '                e = out.setdefault(t, {"portion": 1, "out": {}})   # MUTATION\n',
     'aa364 reprocess_map liest Portion und Ausgang'),
    # ==== REPROCESSING: FORMEL-KANDIDAT + BESTER CHARAKTER (1.0.9) ====
    # Der Skill-Prozentsatz ist eine Zahl, kein Gefuehl.
    ('Reprocessing-Skill zaehlt 2 statt 3 % je Stufe',
     'eve_trader/industry.py',
     'REPRO_JE_STUFE = 0.03\n',
     'REPRO_JE_STUFE = 0.02   # MUTATION\n',
     'aa365 Faktor 5/5/4 + 4 % Implantat'),
    # Regel 3: ueber 100 % ist keine Ausbeute.
    ('Ausbeute ueber 100 % wird durchgereicht',
     'eve_trader/industry.py',
     '    a = b * f\n    return a if a <= 1.0 else None\n',
     '    a = b * f\n    return a   # MUTATION\n',
     'aa365 Ausbeute ueber 100 % ist keine Ausbeute'),
    # Der Struktur-Bonus wirkt multiplikativ, nicht in Prozentpunkten -
    # die Messung vom 18.09.2026 (2439/914/91, 335) unterscheidet beides.
    ('Der Struktur-Bonus wird in Punkten addiert statt multipliziert',
     'eve_trader/industry.py',
     '    a = basis * sec * (1.0 + bonus / 100.0)\n',
     '    a = basis * sec + bonus / 100.0   # MUTATION\n',
     'aa365 MESSUNG Compressed Arkonor 100 -> 2439 Pyerite / 914 Mexallon / 91 Megacyte'),
    # Der Sicherheits-Faktor des Rigs muss rein (Null 1.12).
    ('Der Sicherheits-Faktor des Rigs wird ignoriert',
     'eve_trader/industry.py',
     '    a = basis * sec * (1.0 + bonus / 100.0)\n',
     '    a = basis * (1.0 + bonus / 100.0)   # MUTATION\n',
     'aa365 MESSUNG Compressed Veldspar 100 -> 335 Tritanium'),
    # Das Implantat muss in den Faktor - sonst waehlt der Planer falsch.
    ('Das Reprocessing-Implantat zaehlt nicht mit',
     'eve_trader/industry.py',
     '            * (1.0 + REPRO_ERZ_JE_STUFE * s) * (1.0 + imp / 100.0))\n',
     '            * (1.0 + REPRO_ERZ_JE_STUFE * s))   # MUTATION\n',
     'aa365 das Implantat schlaegt eine Erz-Skill-Stufe'),
    # Gleichstand muss reproduzierbar sein.
    ('Bei Gleichstand gewinnt die groessere ID',
     'eve_trader/industry.py',
     '                                  imp)\n        if f is None:\n            continue\n        if best[1] is None or f > best[1] or (f == best[1] and cid_i < best[0]):\n',
     '                                  imp)\n        if f is None:\n            continue\n        if best[1] is None or f > best[1] or (f == best[1] and cid_i > best[0]):   # MUTATION\n',
     'aa365 Gleichstand -> kleinere ID'),
    # Dasselbe fuer den Scrap-Charakter.
    ('Bei Gleichstand gewinnt der groessere Scrap-Charakter',
     'eve_trader/industry.py',
     '        f = scrap_char_faktor(sk.get(int(sid), 0))\n        if f is None:\n            continue\n        if best[1] is None or f > best[1] or (f == best[1] and cid_i < best[0]):\n',
     '        f = scrap_char_faktor(sk.get(int(sid), 0))\n        if f is None:\n            continue\n        if best[1] is None or f > best[1] or (f == best[1] and cid_i > best[0]):   # MUTATION\n',
     'aa367 bester Scrap-Charakter: hoechste Stufe, Gleichstand kleinere ID, ohne Skill Stufe 0'),
    # Ohne Skill-IDs darf niemand gewaehlt werden - kein Raten.
    ('Ohne Skill-IDs wird trotzdem jemand gewaehlt',
     'eve_trader/industry.py',
     '    if not rid or not eid:\n        return (None, None)\n',
     '    rid = rid or 3385; eid = eid or 3389   # MUTATION\n',
     'aa365 ohne Skill-IDs (SDE zu alt) niemand, kein Raten'),
    # Der Leser muss den Attributnamen mitgeben - sonst weiss spaeter
    # niemand, was der Wert bedeutet.
    ('reprocess_implants verschluckt den Attributnamen',
     'eve_trader/industry.py',
     '                                        "attr": str(r["attr"] or ""),\n',
     '                                        "attr": "",   # MUTATION\n',
     'aa365 reprocess_implants liest den Attributnamen MIT'),
    # Der Tooltip darf die Skill-IDs nicht raten, wenn die SDE sie nicht hat.
    ('Der Reprocessing-Tooltip raet die Skill-IDs',
     'eve_trader/ui/main_window.py',
     '        rid, eid = ids.get("Reprocessing"), ids.get("Reprocessing Efficiency")\n',
     '        rid, eid = ids.get("Reprocessing", 3385), ids.get("Reprocessing Efficiency", 3389)   # MUTATION\n',
     'b14r ohne Skill-IDs aus der SDE keine Zeile (kein Raten)'),
    ('Die Reprocessing-Zeile fehlt im Tooltip',
     'eve_trader/ui/main_window.py',
     '            if _rz:\n                nm_lbl.setToolTip(nm_lbl.toolTip() + "\\n" + _rz)\n',
     '            pass   # MUTATION\n',
     'b14r mit Skills und IDs steht die Zeile im Tooltip'),
    # ==== WEG B: ERZ STATT MINERAL (reprocess.py, 18.09.2026) ====
    # Regel 3: Portionen AUFrunden - 1.04 Portionen sind 2, nicht 1.
    ('Erz-Portionen werden abgerundet',
     'eve_trader/reprocess.py',
     '                portionen = int(math.ceil(need[ziel] / float(q_ziel)))\n',
     '                portionen = max(1, int(need[ziel] // q_ziel))   # MUTATION\n',
     'aa366 Portionen werden AUFgerundet: 350 -> 2 Portionen, 320 Ueberschuss'),
    # Nebenprodukte zaehlen nur, soweit der Plan sie braucht.
    ('Ueberschuss wird als Ersparnis gutgeschrieben',
     'eve_trader/reprocess.py',
     '                    if mat == ziel:\n                        continue\n                    gedeckt = min(q * portionen, need.get(mat, 0))\n',
     '                    if mat == ziel:\n                        continue\n                    gedeckt = q * portionen   # MUTATION\n',
     'aa366 Ueberschuss zaehlt nicht als Ersparnis: kein Tausch'),
    # Ein Tausch, der nichts spart, findet nicht statt.
    ('Auch ein Verlust-Tausch wird gemacht',
     'eve_trader/reprocess.py',
     '                if ersp <= 0.0:\n                    continue\n',
     '                if ersp <= -1e18:\n                    continue   # MUTATION\n',
     'aa366 ... Arkonor fuer 5 Megacyte lohnt nicht (100 x 10000 gegen 15000)'),
    # Ohne SDE-Bonus keine geratene Basis.
    ('Refinery ohne SDE-Eintrag bekommt Bonus 0 statt None',
     'eve_trader/reprocess.py',
     '        if bonus is None:\n            return None, dict(info, grund="sde")\n',
     '        if bonus is None:\n            bonus = 0.0   # MUTATION\n',
     'aa366 Refinery ohne SDE-Eintrag -> None, kein geratener Bonus'),
    # Lowsec ist nicht Null.
    ('Lowsec-Strukturen rechnen mit dem Null-Faktor',
     'eve_trader/reprocess.py',
     '    if v < 2.0:\n        return "low"\n',
     '    if v < 2.0:\n        return "null"   # MUTATION\n',
     'aa366 dieselbe Tatara in Lowsec nimmt 1.06'),
    # Prismaticite hat Zufallsausgang - darf nie Kandidat sein.
    ('Compressed Prismaticite wird Kandidat',
     'eve_trader/reprocess.py',
     '        if "Prismaticite" in nm:\n            continue\n',
     '        pass   # MUTATION\n',
     'aa366 Kandidaten: nur komprimiert, mit Preis, ohne Prismaticite, Kat. 25, mit Ausgang'),
    # ==== WEG B IM DIALOG (b7u) ====
    # Der Tausch muss in der Einkaufsliste ankommen.
    ('Der Erz-Tausch erreicht die Einkaufsliste nicht',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '            neu["buy"] = res["buy"]\n',
     '            pass   # MUTATION\n',
     'b7u der Plan kauft 100 Compressed Testore statt 60 Testmat'),
    # Die Ersparnis muss die Gesamtkosten senken - sonst zwei Wahrheiten.
    ('Die Ersparnis senkt die Gesamtkosten nicht',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '                    neu[key] = float(plan[key]) - ersp\n',
     '                    neu[key] = float(plan[key])   # MUTATION\n',
     'b7u ... und total_cost ist um die Ersparnis gesunken'),
    # Der Schalter muss gespeichert werden.
    ('Der Reprocessing-Schalter wird nicht gespeichert',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '            self.settings["bau_reprocess_on"] = bool(checked)\n',
     '            pass   # MUTATION\n',
     'b7u ... und die Einstellung ist gespeichert'),
    # rebuild() muss den Tausch anwenden, nicht nur der Erst-Aufbau.
    ('rebuild() wendet den Erz-Tausch nicht an',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '                if self._bd_opts.get("reprocess"):\n                    plan = self._reprocess_anwenden(plan, _pfn, names,\n',
     '                if False and self._bd_opts.get("reprocess"):   # MUTATION\n                    plan = self._reprocess_anwenden(plan, _pfn, names,\n',
     'b7u der Plan kauft 100 Compressed Testore statt 60 Testmat'),
    # Ohne SDE-Basis darf nicht mit 0.50 weitergerechnet werden.
    ('Ohne Struktur-Basis wird mit 0.50 gerechnet',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '        basis = ro.get("basis")\n        if basis is None:\n',
     '        basis = ro.get("basis")\n        if basis is None:\n            basis = 0.5   # MUTATION\n        if False:\n',
     "b7u ohne SDE-Daten: kein Tausch, Grund 'sde'"),
    # Der Erst-Aufbau muss den Schalter in die opts bringen.
    ('Der Erst-Aufbau ignoriert den Reprocessing-Schalter',
     'eve_trader/ui/main_window.py',
     '            _ro = self._reprocess_opts()\n            if _ro:\n                opts["reprocess"] = _ro\n',
     '            _ro = None   # MUTATION\n',
     "aa366 der Erst-Aufbau setzt opts['reprocess'] nur mit Schalter"),
    # Eingefrorene Plaene sind gekauft - kein Tausch mehr.
    ('Der Erz-Tausch laeuft auch auf eingefrorene Plaene',
     'eve_trader/ui/main_window.py',
     '            if opts.get("reprocess") and _frozen_plan is None:\n',
     '            if opts.get("reprocess"):   # MUTATION\n',
     'aa366 ... und wendet den Tausch nie auf einen eingefrorenen Plan an'),
    # Stufe 0 muss im Runplaner erscheinen, wenn Erz gekauft wird.
    ('Stufe 0 Reprocessing fehlt im Runplaner',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '            if not _rp0_sch:\n                return\n            _farbe0 = theme.GREEN\n',
     '            if True:   # MUTATION\n                return\n            _farbe0 = theme.GREEN\n',
     'b7u der Runplaner hat Stufe 0 Reprocessing mit der Struktur'),
    # Die Zeile muss die Ausbeute nennen - sonst weiss niemand, was ankommt.
    ('Stufe 0 verschweigt die Ausbeute',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '                        + f"{float(_st0.get(\'ausbeute\') or 0) * 100.0:.1f} %",\n',
     '                        + "",   # MUTATION\n',
     'b7u ... Charakter und Zeile: 100 x Erz -> 60 Testmat (+275), 1 Block, 83.9 %'),
    # Klick auf das Erz muss den Namen kopieren (Nutzer 18.09.2026).
    ('Stufe 0: Erz-Name nicht kopierbar',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '                    _row0.setData(0, ROLLE_KOPIERNAME, _erz_nm0)\n',
     '                    pass   # MUTATION\n',
     'b7u ... Klick auf das Erz kopiert den Erz-Namen (ROLLE_KOPIERNAME)'),
    # Der Mengen-Knopf muss die Menge kopieren, nicht irgendwas.
    ('Stufe 0: Mengen-Knopf kopiert falsche Zahl',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '                            lambda _c=False, _v=_menge0, _n=_erz_nm0:\n                            self._copy_runs_value(_v, _n))\n',
     '                            lambda _c=False, _v=_menge0 + 1, _n=_erz_nm0:   # MUTATION\n                            self._copy_runs_value(_v, _n))\n',
     'b7u ... Klick auf den Knopf legt 100 in die Zwischenablage'),
    # ==== FORTSCHRITT STUFE 0 (Nutzer-Befund 18.09.2026: Einkaufsliste) ====
    # Das Erz muss in den Restbedarf.
    ('rest_anpassen vergisst das Erz',
     'eve_trader/reprocess.py',
     '        if erz and int(st.get("menge") or 0) > 0 and not st.get("gratis"):\n            out[erz] = out.get(erz, 0) + int(st["menge"])\n',
     '        pass   # MUTATION\n',
     'aa366 Restbedarf nicht abgehakt: Erz rein, gedecktes Mineral raus'),
    # Ein abgehakter Schritt darf nichts mehr gutschreiben.
    ('fehl_anpassen ignoriert den Haken',
     'eve_trader/reprocess.py',
     ...
     '    for st in (schritte or []):\n        if st.get("built"):   # MUTATION\n            continue\n        for m, q in (st.get("deckt") or {}).items():\n            kredit[int(m)]',
     'aa366 Fehlbedarf abgehakt: kein Kredit, kein Erz'),
    # Der Fehlbedarf des Dialogs muss den Tausch kennen.
    ('_fehlbedarf_jetzt kennt das Reprocessing nicht',
     'eve_trader/ui/main_window.py',
     '        return reprocess.fehl_anpassen(\n            _fehl, ((plan.get("reprocess") or {}).get("schritte") or []),\n',
     '        return reprocess.fehl_anpassen(\n            _fehl, [],   # MUTATION\n',
     'b7u Fehlbedarf jetzt: nur das Erz fehlt (100), Testmat ist gedeckt'),
    # Der Restbedarf der Einkaufsliste ebenso.
    ('_restbedarf_jetzt kennt das Reprocessing nicht',
     'eve_trader/ui/main_window.py',
     '        return reprocess.rest_anpassen(\n            need, ((plan.get("reprocess") or {}).get("schritte") or []), _checked)\n',
     '        return need   # MUTATION\n',
     'b7u Restbedarf: 100 Erz + 40 Testmat (100 Bedarf - 60 gedeckt), kein Kauf'),
    # Gedeckte Minerale muessen im Reiter stehen.
    ('Gedeckte Minerale fehlen im Materialien-Reiter',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '            reprocessed = _rp_deckt.get(tid, 0) or 0\n',
     '            reprocessed = 0   # MUTATION\n',
     'b7u Testmat-Zeile: 60 aus Reprocessing, 0 zu kaufen, 100 Bedarf'),
    # Der Haken der Stufe-0-Zeile muss im Plan landen (Schluessel).
    ('Der Stufe-0-Haken hat keinen Schluessel',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '                    _row0.setData(0, Qt.UserRole + 6, _key0)\n',
     '                    pass   # MUTATION\n',
     'b7u der Haken landet als repro|62516 im Plan'),
    # Der Rezept-Baum muss das Erz nennen, wenn das Mineral daraus kommt.
    ('Der Rezept-Baum verschweigt das komprimierte Erz',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '                if aq > 0 and _rp_b and _by_b is not None and _cid_b not in _by_b:\n',
     '                if False and aq > 0 and _rp_b and _by_b is not None and _cid_b not in _by_b:   # MUTATION\n',
     "b7u der Rezept-Baum sagt bei Testmat 'aus komprimiertem Erz'"),
    # ==== FRACHT IN DER LADDER (b7v, 18.09.2026) ====
    ('Die Orderbuch-Ladder vergisst den Frachtdienst',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '                if _fr_rate > 0:\n                    mat_ladder += _fr_in_mat\n',
     '                pass   # MUTATION\n',
     'b7v Gesamt-Baukosten ENTHALTEN die Fracht'),
    # "Warum nicht" muss das beste Erz nennen, nicht das erste.
    ('abgelehnt nennt das erste statt das guenstigste Erz',
     'eve_trader/reprocess.py',
     '            if pct is not None and (best_pct is None or pct < best_pct):\n',
     '            if pct is not None and best_pct is None:   # MUTATION\n',
     'aa366 abgelehnt nennt das guenstigste Erz (5000 vor 10000)'),
    # "Batch Compressed" muss Kandidat bleiben (Nutzer 18.09.2026).
    ('Batch Compressed faellt wieder raus',
     'eve_trader/reprocess.py',
     '        if not (nm.startswith("Compressed ") or nm.startswith("Batch Compressed ")):\n',
     '        if not nm.startswith("Compressed "):   # MUTATION\n',
     'aa366 Batch Compressed ist Kandidat'),
    # Die Knappheit muss im Tab ankommen, nicht nur im Tooltip.
    ('Die Knappheitszeile wird nach der Ladder nicht nachgezogen',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '                try:\n                    self._knappheit_nachziehen(names)\n',
     '                try:\n                    pass   # MUTATION\n',
     'b7v der Materialien-Tab warnt: Testmat 45 / 60 am Hub'),
    # ==== REPROCESSING-IMPLANTAT (18.09.2026) ====
    # Das erkannte Implantat muss in die Ausbeute.
    ('Das Reprocessing-Implantat erreicht die Ausbeute nicht',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '                basis, self.settings.get("bau_char_skills", {}) or {},\n                self._reprocess_implants_map(),\n',
     '                basis, self.settings.get("bau_char_skills", {}) or {},\n                {},   # MUTATION\n',
     'b7u ... und die Ausbeute steigt: 348 je Portion -> Ueberschuss 288'),
    # Das beste Implantat zaehlt, nicht das erste gefundene.
    ('Der Implantat-Abruf speichert nichts',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '            self.settings["bau_char_reproc_implant"] = out\n',
     '            pass   # MUTATION\n',
     'b7u das Implantat ist gespeichert (RX-804, 4 %)'),
    # ==== WEG A: UNREFINED-REAKTIONEN (19.09.2026) ====
    # Nur genau EIN Fremd-Ausgang macht einen Kandidaten - sonst waere V
    # (zwei Fremd-Ausgaenge) ploetzlich einer.
    ('Unrefined-Kandidat auch mit zwei Fremd-Ausgaengen',
     'eve_trader/reprocess.py',
     '        if len(fremd) != 1:\n            continue\n        x = fremd[0]\n',
     '        if len(fremd) < 1:   # MUTATION\n            continue\n        x = fremd[0]\n',
     'aa367 ... mit Formel, Inputs, Ausgang je Run und Ruecklaeufer'),
    # Die Ausbeute muss in den Ausgang je Run - nicht 100 %.
    ('Unrefined-Ausgang je Run ignoriert die Ausbeute',
     'eve_trader/reprocess.py',
     '        erg = industry.reprocess_ergebnis(k["je_run"], 1, 1, a)\n',
     '        erg = industry.reprocess_ergebnis(k["je_run"], 1, 1, 1.0)   # MUTATION\n',
     'aa367 21 X je Run, 98 A zurueck, Charakter 7'),
    # Der Ruecklaeufer zaehlt in der Entscheidung (Nutzer-Entscheid).
    ('Unrefined-Wahl vergisst die Ruecklaeufer-Gutschrift',
     'eve_trader/reprocess.py',
     '            kredit += q * pk\n            ruecklaeufer.append((int(m), int(q), pk))\n',
     '            kredit += 0.0   # MUTATION\n            ruecklaeufer.append((int(m), int(q), pk))\n',
     "aa367 Unrefined gewinnt: 357.14 je X gegen 10'502.5 normal"),
    # Nur STRIKT guenstiger gewinnt.
    ('Unrefined-Wahl gewinnt auch, wenn teurer',
     'eve_trader/reprocess.py',
     '        if per_unit < vergleich:\n            wahl[x] = dict(k, per_unit=per_unit, vergleich=vergleich, kredit_je_run=kredit)\n',
     '        if per_unit > vergleich:   # MUTATION\n            wahl[x] = dict(k, per_unit=per_unit, vergleich=vergleich, kredit_je_run=kredit)\n',
     "aa367 Unrefined gewinnt: 357.14 je X gegen 10'502.5 normal"),
    # Die Rezept-Kopie darf das Original (Prozess-Cache) nicht anfassen.
    ('Rezept-Kopie schreibt ins Original',
     'eve_trader/reprocess.py',
     '    neu.product_to_bp = dict(recipes.product_to_bp)\n',
     '    neu.product_to_bp = recipes.product_to_bp   # MUTATION\n',
     'aa367 ... Original unveraendert'),
    # Die Gutschrift SENKT die Gesamtkosten.
    ('Ruecklaeufer-Gutschrift erhoeht die Kosten',
     'eve_trader/reprocess.py',
     '            neu["total_cost"] = float(neu["total_cost"]) - gesamt\n',
     '            neu["total_cost"] = float(neu["total_cost"]) + gesamt   # MUTATION\n',
     'aa367 ... total_cost minus Gutschrift, mat_cost unveraendert, Wert genannt'),
    # RUNS JE JOB (Nutzer 19.09.2026, Einherji II: "17 Stueck mit einem
    # Blueprint ... gibts maximal 10 runs"): der Deckel muss die Runs eines
    # Slots in Jobs zerlegen - sonst steht wieder 1 x 17.
    ('Runs-Deckel je Job wird ignoriert',
     'eve_trader/industry.py',
     '            if _m and _R0 >= 1:\n',
     '            if False:   # MUTATION\n',
     'aa368 mit Deckel 10: 17 Jobs a genau 10 Runs, Summe bleibt 170'),
    # Jede Kopie zaehlt bei den Wellen mit (zwei Starts auf einem Slot).
    ('Kopien zaehlen nicht bei den Wellen',
     'eve_trader/industry.py',
     '            for _ch in _chunks:\n                dur = _ch * _tv\n',
     '            for _ch in _chunks[:1]:   # MUTATION\n                dur = int(round(r)) * _tv\n',
     'aa368 zwei Wellen (10 Starts, dann 7 Starts), jede so lang wie eine Kopie'),
    # Der Rest (letzte Einheit) muss die Runs des Slots senken - sonst
    # stehen 180 Runs statt 175 im Plan.
    ('Rest-Kopie wird als volle Kopie gezaehlt',
     'eve_trader/industry.py',
     '                    runs_k[_kr] -= (_m - _rem)\n',
     '                    runs_k[_kr] -= 0   # MUTATION\n',
     'aa368 175 Runs: 17 volle Kopien + eine mit 5 (nur der Rest bleibt angefangen)'),
    # Die Anzeige darf nie ueber den Deckel teilen.
    ('Anzeige-Aufteilung ignoriert den Deckel',
     'eve_trader/ui/mw_helpers.py',
     '        if m >= 1:\n            parts = [m] * (R // m) + ([R % m] if R % m else [])\n',
     '        if False:   # MUTATION\n            parts = [m] * (R // m) + ([R % m] if R % m else [])\n',
     'aa368 kein Teil ueber dem Deckel, auch wenn der Planer weniger Jobs nannte'),
    # Der Runplaner muss den Deckel auch UEBERGEBEN.
    ('Runplaner uebergibt keinen Runs-Deckel',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '            per_item_runs_cap=self._resolve_per_item_runs_cap(type_id))',
     '            per_item_runs_cap={})   # MUTATION',
     'b7w Runplaner mit Limit 4 je Job, 1 Slot: Welle 1 = eine Kopie mit 4 Runs'),
    # Die kleinste eigene Kopie zaehlt (Regel 3), nicht die groesste.
    ('BPC-Deckel nimmt die groesste Kopie',
     'eve_trader/ui/mw_helpers.py',
     '                min_runs[_bid] = min(min_runs.get(_bid, _r), _r)\n',
     '                min_runs[_bid] = max(min_runs.get(_bid, _r), _r)   # MUTATION\n',
     'aa368 BPC-Runs je Item: 901 -> 4 (kleinste), BPO 900 hebt auf, 0 Runs zaehlt nicht'),
    # Der Platzhalter 1x1 (vor dem ersten rebuild) darf NICHT deckeln.
    ('Platzhalter-Karte deckelt auf 1 Run je Job',
     'eve_trader/ui/mw_helpers.py',
     '        if end_tid is not None and _end.get("runs_known") and not _end.get("bpo"):\n',
     '        if end_tid is not None and not _end.get("bpo"):   # MUTATION\n',
     'aa368 der Platzhalter 1x1 VOR dem ersten rebuild (ohne runs_known) zaehlt NICHT'),
    # Die Unrefined-Stufe rechnet ihre Wellen mit Reaktions-Slots.
    ('Unrefined-Wellen mit Fertigungs-Slots',
     'eve_trader/industry.py',
     '        cslots = (react_c if (stage.startswith("reaction") or stage == "unrefined")\n                  else mfg_c)\n',
     '        cslots = (react_c if stage.startswith("reaction")   # MUTATION\n                  else mfg_c)\n',
     'aa368 Unrefined-Wellen rechnen mit Reaktions-Slots (3 Jobs = 1 Welle)'),
    # EIN CHARAKTER FUER ALLES (Nutzer 19.09.2026): Gewicht = Menge x Preis.
    ('Ein-Charakter-Wahl ignoriert den Einkaufswert',
     'eve_trader/reprocess.py',
     '        gewicht[int(erz)] = gewicht.get(int(erz), 0.0) + float(st.get("menge") or 0) * (p or 1.0)\n',
     '        gewicht[int(erz)] = gewicht.get(int(erz), 0.0) + float(st.get("menge") or 0)   # MUTATION\n',
     'aa366 ein Charakter: bei gleicher Menge entscheidet der Einkaufswert - Arkonor teurer -> 2'),
    ('ausbeute_funktion ignoriert den festen Charakter',
     'eve_trader/reprocess.py',
     '    if fest is not None:\n        skills_by_char = {str(k): v for k, v in (skills_by_char or {}).items()\n',
     '    if False:   # MUTATION\n        skills_by_char = {str(k): v for k, v in (skills_by_char or {}).items()\n',
     'aa366 ausbeute_funktion(fest=2): auch Veldspar bei Charakter 2 (Simple 0 -> 0.76231)'),
    # Neuer Plan: beide Reprocessing-Haken AUS (Nutzer 19.09.2026).
    ('Neuer Plan behaelt die Reprocessing-Haken',
     'eve_trader/ui/main_window.py',
     '                            "bau_buy_datacores", "bau_buy_decryptors",\n                            "bau_reprocess_on", "bau_unrefined_on"):',
     '                            "bau_buy_datacores", "bau_buy_decryptors"):   # MUTATION',
     'aa352 bau_reprocess_on wird zurueckgesetzt'),
    # Haken am Charakter in Stufe 0.
    # (Die Flag allein ist kein Kaestchen - Qt hakt jede Zeile von Haus aus,
    # sichtbar wird es erst mit CheckStateRole. Deshalb faellt hier der
    # setCheckState-Aufruf, nicht die Flag - s. CLAUDE.md, Qt-Fallen.)
    ('Stufe-0-Charakter ohne Kaestchen',
     'eve_trader/ui/mw_bauplan_tabs.py',
    ...
    # WELLEN (Nutzer 19.09.2026): mehr Kopien als Slots -> zweite Auflistung.
    ('Runplaner listet keine zweite Welle',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '                if _jobs <= cap or not all(a.get("parts") for a in alist):\n                    return [(1, alist)]\n',
     '                if True:   # MUTATION\n                    return [(1, alist)]\n',
     'b7w ... Welle 2 = zweite Auflistung des Charakters mit der Kopie 2'),
    ('Welle-2-Haken teilen den Schluessel mit Welle 1',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '                    _ckey_item = f"{stage}|{cid}|{a[\'tid\']}{_wsuf}"',
     '                    _ckey_item = f"{stage}|{cid}|{a[\'tid\']}"   # MUTATION',
     'b7w ... die Welle-2-Zeile hat einen eigenen Haken-Schluessel (|w2)'),
    # Ganze Kopien duerfen ALLE Slots aller Charaktere nutzen.
    ('Kopien bleiben auf zwei Charaktere gedeckelt',
     'eve_trader/industry.py',
     '                return R if tid in _unit else max_reasonable\n',
     '                return max_reasonable   # MUTATION\n',
     'aa368 310 Runs auf 9+10+5 Slots: alle 24 Slots voll, Rest 7 als zweite Welle bei Leziris'),
    ...
    ('Corp-Hangars standardmaessig keiner',
     ...
     '    "corp_divisions": [],   # MUTATION',
     'aa362 standardmaessig sind alle sieben Corp-Hangars gewaehlt'),
    # build_cost muss die Ruecklaeufer-Gutschrift kennen (Thulium-Hafnite-Fund).
    ('build_cost ohne Ruecklaeufer-Gutschrift',
     'eve_trader/industry.py',
     '        if _kr > 0:\n            per_unit = max(0.0, per_unit - _kr)\n',
     '        if False:   # MUTATION\n            per_unit = max(0.0, per_unit - _kr)\n',
     'aa367 build_cost ueber die Kopie rechnet die Gutschrift ein: 357.14 je X'),
    ('production_plan-Schaetzung ohne Ruecklaeufer-Gutschrift',
     'eve_trader/industry.py',
     '        if _uw and runs > 0:\n            c = max(0.0, c - runs * float(_uw.get("kredit_je_run") or 0.0))\n',
     '        if False:   # MUTATION\n            c = max(0.0, c - runs * float(_uw.get("kredit_je_run") or 0.0))\n',
     "aa367 Kaufpreis 3'000 (unter 5'023 ohne, ueber 357 mit Gutschrift): Plan BAUT X"),
    # EC-Rollenbonus fuer invented Items (Viator-Befund 19.09.2026).
    ('invented-Pfad ohne EC-Rollenbonus',
     'eve_trader/industry.py',
     '            * (1 - float(ec_me or 0) / 100.0)) * 100.0\n',
     '            * 1.0) * 100.0   # MUTATION\n',
     "aa369 mit EC 1 %: 57'620 Plates (statt 58'200), 740 Fusion, 2'180 Ion"),
    ('production_plan-Pfad nimmt ec_me_map nicht',
     'eve_trader/industry.py',
     '                pct = me_invented_pct(inv_me, rig_me,\n                                      (opts.get("ec_me_map") or {}).get(tid, 0) or 0)\n',
     '                pct = me_invented_pct(inv_me, rig_me, 0)   # MUTATION\n',
     "aa369 mit EC 1 %: 57'620 Plates (statt 58'200), 740 Fusion, 2'180 Ion"),
    ('Dialog reicht ec_me_map nicht weiter',
     'eve_trader/ui/main_window.py',
     '                opts["ec_me_map"] = ec_me_map          # EC-Rollenbonus fuer invented Items\n',
     '                pass   # MUTATION\n',
     'aa369 drei Aufrufer setzen ec_me_map in die opts'),
    # Corp-Orte in der Ortssuche (Tester-Befund 19.09.2026).
    ('corp.orte laeuft nicht zur Wurzel hoch',
     'eve_trader/corp.py',
     '        loc = wurzel_ort(a, by_id)\n        try:\n            loc = int(loc or 0)\n',
     '        loc = a.get("location_id")   # MUTATION\n        try:\n            loc = int(loc or 0)\n',
     'aa362 corp.orte: Wurzel je Item - Citadel ueber das Buero, Station direkt, Container hoch'),
    ('Ortssuche ignoriert die Corp-Orte',
     'eve_trader/ui/main_window.py',
     '                sids = set(sids) | {_o for _o, _c in _corp_orte.items()\n                                    if _c == cid and _o >= 1_000_000_000_000}\n',
     '                sids = set(sids)   # MUTATION\n',
     'aa362 die Ortssuche nimmt Corp-Orte dazu (Strukturen UND Stationen)'),
    # Gebaute Schritte duerfen den Restbedarf nicht anfassen (U wird nie gekauft).
    ('rest_anpassen kauft das Unrefined-Produkt',
     'eve_trader/reprocess.py',
     '        if schritt_key(st) in abgehakt or st.get("built"):\n            continue           # Weg A: das Unrefined-Produkt wird gebaut, nie gekauft\n',
     '        if schritt_key(st) in abgehakt:   # MUTATION\n            continue\n',
     'aa367 rest_anpassen ignoriert den gebauten Schritt'),
    # Der Schalter muss in den opts ankommen.
    ('Weg-A-Schalter kommt nicht in den opts an',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '        weg_a = bool(self.settings.get("bau_unrefined_on"))\n',
     '        weg_a = False   # MUTATION\n',
     'b7w opts: Weg A an, Weg B aus, gemessene Basis'),
    # Ohne Rezept-Kopie laeuft X weiter ueber die normale Formel.
    ('Die Rezept-Kopie wird nie gebaut',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '        if not ro.get("unrefined"):\n            return recipes_basis, {}\n',
     '        if True:   # MUTATION\n            return recipes_basis, {}\n',
     'b7w die Wahl faellt auf X (19 X + 53 A je Run, Charakter 1)'),
    # Der Blaupausen-Name muss die Unrefined-Formel nennen.
    ('Blaupausen-Name ignoriert die Unrefined-Formel',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '        if not _uw:\n            return name\n        _nm = (getattr(self, "_bd_names_ref", None) or {}).get(int(_uw.get("u") or 0))\n',
     '        if True:   # MUTATION\n            return name\n        _nm = (getattr(self, "_bd_names_ref", None) or {}).get(int(_uw.get("u") or 0))\n',
     'b7w der Reaktions-Job traegt den Namen der Unrefined-Formel zum Kopieren'),
    # Der Block gehoert NACH die Reaktionsstufe.
    ('Unrefined-Block fehlt im Runplaner',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '            if stage == _ub_nach:\n                _unref_block()\n',
     '            if False and stage == _ub_nach:   # MUTATION\n                _unref_block()\n',
     'b7w der Runplaner hat den Unrefined-Block direkt nach der Reaktionsstufe'),
    # Der Rezeptbaum muss den Weg nennen.
    ('Rezeptbaum verschweigt die Unrefined-Formel',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '                    if _uw_t:\n                        act += "  \\u00b7  " + _txt("via {formula} \\u267b").format(\n',
     '                    if False and _uw_t:   # MUTATION\n                        act += "  \\u00b7  " + _txt("via {formula} \\u267b").format(\n',
     "b7w der Rezept-Baum sagt bei Testmat 'BUILD - 6 runs - via Unrefined Testmat'"),
    # Die Kostenzeile muss erscheinen, wenn es einen Ruecklaeufer gibt.
    ('Kostenzeile Ruecklaeufer bleibt versteckt',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '                    _rl_lbl.setVisible(bool(_rl_w))\n',
     '                    _rl_lbl.setVisible(False)   # MUTATION\n',
     "b7w die Kostenzeile 'Ruecklaeufer' ist sichtbar und nennt 318'000"),
    # Der Scrapmetal-Pfad hat eine feste 50-%-Basis (gemessen).
    ('Scrapmetal-Ausbeute nimmt eine andere Basis',
     'eve_trader/industry.py',
     '    return reprocess_ausbeute(REPRO_SERVICE_BASIS, char_faktor)\n',
     '    return reprocess_ausbeute(0.55, char_faktor)   # MUTATION\n',
     'aa367 Scrapmetal-Ausbeute 53,0 % - unabhaengig von Struktur und Rig'),
    # Weg A muss ueber den Scrapmetal-Pfad rechnen, nicht ueber den Erz-Pfad.
    ('Weg A rechnet mit der Struktur-Basis statt Scrapmetal',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '            af = reprocess.scrap_ausbeute_funktion(\n                self.settings.get("bau_char_skills", {}) or {},\n                industry.reprocess_skill_ids())\n',
     '            af = reprocess.ausbeute_funktion(   # MUTATION\n                ro.get("basis") or 0.5, self.settings.get("bau_char_skills", {}) or {},\n                self._reprocess_implants_map(),\n                industry.reprocess_skill_ids(), industry.reprocess_erz_skill())\n',
     'b7w die Wahl faellt auf X (19 X + 53 A je Run, Charakter 1)'),
    # Ein Input, den der Plan nicht bauen darf, zaehlt zum Kaufpreis.
    ('Unrefined-Wahl rechnet never_build-Inputs zum Baupreis',
     'eve_trader/reprocess.py',
     '        b = None if int(tid) in never else industry.build_cost(tid, price_fn, recipes, marg, memo)\n',
     '        b = industry.build_cost(tid, price_fn, recipes, marg, memo)   # MUTATION\n',
     'aa367 ... in never_build zaehlt er zum Kaufpreis (357.14 wie ohne Rezept)'),
    # Die Gutschrift muss auch nach "Recalculate" (Ladder-Zweig) in der Summe sein.
    ('Ladder-Summe vergisst die Ruecklaeufer-Gutschrift',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '                total -= float(((plan or {}).get("reprocess") or {}).get(\n                    "ruecklaeufer_wert") or 0.0)\n',
     '                total -= 0.0   # MUTATION\n',
     'b7w Ladder: Gesamt-Baukosten ziehen die Gutschrift ab'),
    # Nach dem Schalter muessen die Orderbuecher nachgeholt werden.
    ('Reprocessing-Schalter holt keine Orderbuecher nach',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '            if (self._bd_ladder_ctx(qty_spin.value()) is not None\n                    and not getattr(self, "_bd_frozen", None)):\n                try:\n                    _ladder_refresh()\n',
     '            if False:   # MUTATION\n                try:\n                    _ladder_refresh()\n',
     'b7w jeder Schalter-Klick holt die Orderbuch-Preise nach (2 Klicks = 2 Abrufe)'),
    # ==== ERZ SICHTBAR: Gruppe, Baum, Karte, Blacklist (19.09.2026) ====
    ('Blacklist-Erz landet doch auf der Einkaufsliste',
     'eve_trader/reprocess.py',
     '        if erz not in gratis:\n            buy[erz] = buy.get(erz, 0) + portionen * portion\n',
     '        if True:   # MUTATION\n            buy[erz] = buy.get(erz, 0) + portionen * portion\n',
     "b7u Erz auf der Blacklist: Einkaufsliste leer, Schritt gratis, deckt 60 Testmat, Ersparnis 6'000"),
    ('Blacklist-Erz kostet in der Wahl doch etwas',
     'eve_trader/reprocess.py',
     '    def _preis_erz(erz):\n        return 0.0 if erz in gratis else _preis(erz)\n',
     '    def _preis_erz(erz):\n        return _preis(erz)   # MUTATION\n',
     "b7u Erz auf der Blacklist: Einkaufsliste leer, Schritt gratis, deckt 60 Testmat, Ersparnis 6'000"),
    ('Gratis-Erz steht doch im Runplaner',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '                    if _s.get("art") != "unrefined" and not _s.get("gratis")]\n',
     '                    if _s.get("art") != "unrefined"]   # MUTATION\n',
     'b7u ... und der Runplaner hat KEINE Stufe 0'),
    ('Gratis-Erz landet im Restbedarf',
     'eve_trader/reprocess.py',
     '        if erz and int(st.get("menge") or 0) > 0 and not st.get("gratis"):\n            out[erz] = out.get(erz, 0) + int(st["menge"])\n',
     '        if erz and int(st.get("menge") or 0) > 0:   # MUTATION\n            out[erz] = out.get(erz, 0) + int(st["menge"])\n',
     'b7u ... Restbedarf ohne Erz, Testmat gedeckt'),
    ('Der Rezept-Baum hat keinen Erz-Kopf',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '            if _erz_steps:\n                # de_scan5: aus - Kategorie-SCHLUESSEL, Anzeige via _kategorie_anzeige\n',
     '            if False and _erz_steps:   # MUTATION\n                # de_scan5: aus - Kategorie-SCHLUESSEL, Anzeige via _kategorie_anzeige\n',
     "b7u der Rezept-Baum hat den Kopf 'Compressed ore (1)' mit der Erz-Zeile"),
    ('Die Erz-Zeile verschweigt Ausgang und Ausbeute',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '            if _rp_zeile:\n                status_txt = _rp_zeile + "  \\u00b7  " + status_txt\n',
     '            if False:   # MUTATION\n                status_txt = _rp_zeile + "  \\u00b7  " + status_txt\n',
     "b7u die Erz-Zeile steht unter 'Compressed ore' und nennt 60 Testmat, 83.9 %, Peanut Motor"),
    ('Erz landet nicht in der Gruppe Erz',
     'eve_trader/ui/main_window.py',
     '        if _inf and _inf[0] == 25:\n',
     '        if _inf and _inf[0] == -25:   # MUTATION\n',
     "b7u die Erz-Zeile steht unter 'Compressed ore' und nennt 60 Testmat, 83.9 %, Peanut Motor"),
    ('Die Karte nennt die Ersparnis nicht',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '                if _z:\n                    rp_lbl.setText((rp_lbl.text() + "\\n" if rp_lbl.text() else "") + _z)\n',
     '                if False:   # MUTATION\n                    rp_lbl.setText((rp_lbl.text() + "\\n" if rp_lbl.text() else "") + _z)\n',
     "b7u die Karte nennt die Ersparnis 5'900 und 1 Erz"),
    # Gestelltes Erz muss im Materials-Tab sichtbar bleiben (Menge!).
    ('Gestelltes Erz verschwindet aus dem Materials-Tab',
     'eve_trader/ui/mw_bauplan_tabs.py',
     '            total = missing + used + built + reprocessed + gratis_q\n',
     '            total = missing + used + built + reprocessed   # MUTATION\n',
     'b7u ... der Materials-Tab zeigt das Erz trotzdem: 100 gestellt, nichts fehlt'),
    ...
    ('Der Build-or-buy-Schritt stellt seinen Reiter nicht mehr selbst her',
     ...
     '          "_bd_karte_tiefe"),\n'
     '         t("Build or buy?"),\n',
     '        (("_bd_karte_bauenkaufen", "_bd_karte_tiefe"),\n'
     '         t("Build or buy?"),\n',
     'b78 und stellt dabei den Rezeptstruktur-Reiter selbst her'),
    ('Die Gewinn-Uebersicht schneidet lange Namen wieder hart ab',
     'eve_trader/ui/main_window.py',
     '            nm = ElideLabel(p.get("label") or p.get("item_name") or "?")\n',
     '            nm = QLabel(p.get("label") or p.get("item_name") or "?")   # MUTATION\n',
     'b2t in der Gewinn-Uebersicht ist der lange Name ein ElideLabel'),
    ('Alte Plan-Namen behalten das Reagenzglas',
     'eve_trader/config.py',
     '            if isinstance(_v, str) and plan_name_bereinigen(_v) != _v:\n',
     '            if False:   # MUTATION\n',
     'aa372 die Migration bereinigt label UND item_name'),
    ('Beim Speichern rutscht das Reagenzglas wieder in den Namen',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '            label = config.plan_name_bereinigen(label)\n',
     '            label = (label or "").strip()\n',
     'aa372 beim Speichern werden label und item_name bereinigt'),
    ('Die Produktionstiefe blinkt im Tutorial nicht mehr mit',
     'eve_trader/ui/tutorial.py',
     '        (("bd:tab:" + t("Recipe structure"), "_bd_karte_bauenkaufen",\n'
     '          "_bd_karte_tiefe"),\n',
     '        (("bd:tab:" + t("Recipe structure"), "_bd_karte_bauenkaufen"),\n',
     "b78 'Build or buy' laesst auch 'Production depth' blinken"),
    ...
    # FEHLER.LOG-NETZ (18.09.2026): der Unrefined-Namensnachzug griff auf
    # self._bd_groups zu, bevor open_build_detail es gesetzt hatte - sechs
    # AttributeErrors je Prueflauf, nur in fehler.log sichtbar. b79 liest
    # den Zuwachs von fehler.log und faengt so JEDEN abgefangenen
    # Programmierfehler, nicht nur diesen.
    ('Der Unrefined-Namensnachzug verlangt wieder ein gesetztes _bd_groups',
     'eve_trader/ui/mw_bauplan_fenster.py',
     '                    _gm_u = getattr(self, "_bd_groups", None)\n'
     '                    if _gm_u is None:\n'
     '                        _gm_u = self._bd_groups = {}\n',
     '                    _gm_u = self._bd_groups   # MUTATION\n',
     'b79 fehler.log: keine Programmierfehler waehrend des Laufs angehaengt'),
    ...
    ('Die Statuszeile zeigt beim Entpacken wieder -1/0 MB',
     'eve_trader/ui/main_window.py',
     '                t("Unpacking and importing \\u2026") if d < 0 else\n',
     '',
     'aa370 load_sde uebersetzt die Entpack-Phase'),
    # --- b80: Preisverlauf, Vervollstaendigung ueber alle Namen (18.09.2026) ---
    ('Der Completer haengt wieder nur an der Bestands-Combo',
     'eve_trader/ui/main_window.py',
     '        comp = QCompleter(self._mk_namen_modell, self.mk_item)\n',
     '        comp = self.mk_item.completer()   # MUTATION\n',
     'b80 der Completer haengt am Namens-Modell'),
    ('Getippte Namen gehen wieder immer ueber ESI',
     'eve_trader/ui/main_window.py',
     '            if _tid_lok is not None:\n',
     '            if False:   # MUTATION\n',
     'b80 getippter Name wird lokal aufgeloest'),
    ('Der Tab-Wechsel startet keinen Graphen mehr',
     'eve_trader/ui/main_window.py',
     '        if cur is getattr(self, "market_tab", None):\n            self._mk_auto_start()\n',
     '',
     'b80 der Tab-Wechsel ruft den Auto-Start'),
    ('Der Ladeschirm zeigt beim Entpacken wieder -1',
     'eve_trader/ui/main_window.py',
     '        elif done < 0:\n',
     '        elif False:   # MUTATION\n',
     'b81 Ladeschirm: -1 wird als Entpack-Phase gezeigt'),
    ('Die Taskleiste zeigt wieder das Python-Symbol',
     'eve_trader/__main__.py',
     '            ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(\n                "PeanutMotor.EVEMotorMarket")\n',
     '            pass   # MUTATION\n',
     'aa371 eigene AppUserModelID wird gesetzt'),
    ('Ohne gemerktes Item bleibt der Preisverlauf wieder leer',
     'eve_trader/ui/main_window.py',
     '            self.mk_item.setEditText(self._MK_STARTITEM)\n            self._plot_history()\n            return\n',
     '            return   # MUTATION\n',
     'b80 ohne gemerktes Item: Acolyte II'),
    ('Der Auto-Start laeuft bei jedem Tab-Wechsel neu',
     'eve_trader/ui/main_window.py',
